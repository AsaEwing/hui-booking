...
export async function onRequestGet({ request, env }) {
    ...
    const { results } = await env.DB.prepare(
        'SELECT id, name, floorplan_url, walls_json, wall_count, is_active, active_until, is_open, open_at, close_at, pref_count, max_combo_size, drive_url, created_at FROM projects ORDER BY created_at DESC'
    ).all();
...
export async function onRequestPost({ request, env }) {
    ...
    const { name, floorplan_url, floorplan_data, floorplan_mime, walls, open_at, close_at, active_until, pref_count, max_combo_size, is_open, drive_url } = body;
    ...
    const result = await env.DB.prepare(
        'INSERT INTO projects (name, floorplan_url, floorplan_data, floorplan_mime, walls_json, wall_count, open_at, close_at, active_until, pref_count, max_combo_size, is_open, is_active, drive_url, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,0,?,?)'
    ).bind(name.trim(), floorplan_url || '/floorplan.jpg', floorplan_data || null, floorplan_mime || null, JSON.stringify(walls), Object.keys(walls).length, open_at || null, close_at || null, active_until || null, pc, mcs, is_open ? 1 : 0, driveVal, now).run();
