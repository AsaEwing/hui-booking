...
- 補位/移動單一學生時指派的位置數量，跟學生自己填志願一樣受專案的組合位置上限（`max_combo_size`）限制，後端與前端都會檢查
- **交換**兩位學生的位置時，後端用 D1 batch 一次寫入兩筆紀錄，避免只換一半
- 調整後的最終結果會顯示在**所有**位置頁面，包含公開的 `result.html`（因為這才是真正的最終結果），手動調整過的位置用**菱形標記 + 紫色**跟正式抽籤/時間排序結果的圓形標記區分，平面圖、學生列表、位置列表都看得到
- 行內的「◆」標記只有 hover 提示固定文字（不含原因，且手機無法 hover）；平面圖圖例旁的「查看調整紀錄」按鈕會展開完整表格（學生、調整為、原因、時間），**後台與公開頁面都有**——公開頁面不需登入即可查看，但不顯示操作者身分，這部分只有後台看得到
...
後台與 `result.html` 的平面圖區域都有「下載目前畫面（JPEG）」按鈕，用瀏覽器的 `<canvas>` 把平面圖底圖跟目前畫面上的 SVG 標記層（含姓名、手動調整的菱形標記）合成成一張圖片下載，解析度跟原圖一致。

- **只有本站上傳的圖片才會顯示這顆按鈕**（`floorplan_url` 是 `/` 開頭的同源相對路徑，也就是後台說明裡「本站檔案」那種）。Google Drive／Imgur 這類外部連結的圖片畫進 `canvas` 後會被瀏覽器標記為「受污染」，匯出時會直接被擋下，所以這種來源不提供下載按鈕
- 格式固定用 JPEG，是所有瀏覽器裝置對 `canvas` 匯出支援度最一致、相容性最好的格式
- 手機瀏覽器長按圖片用內建的「儲存圖片」功能，存到的是**最原始、沒有任何標記的平面圖檔案**（完全繞過這個合成邏輯），要存含標記的版本一定要點這顆按鈕
...
- 設定學生的「顯示名稱」，覆蓋平面圖標記上顯示的姓名——因為學生註冊時不強制使用本名，可能跟正式展覽需要的姓名不同。只影響平面圖標記（含下載的合成圖片），不影響學生列表、CSV 匯出、匿名遮蔽邏輯；未登入的公開訪客一律照舊看到遮蔽後的原始姓名，只有登入者（含管理員）看平面圖時才會看到顯示名稱，留空則照舊用原始姓名
