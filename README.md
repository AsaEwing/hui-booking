...
3. **自己抽**：讓使用者選擇一個目前**尚未抽籤**的抽籤制專案，帶入該專案實際登記的志願資料模擬抽籤——姓名一律匿名化為「學生1」「學生2」等代稱，畫面最上方有醒目紅字警語提醒這只是練習模擬、非正式結果。這個分頁同時會用精確組合數學（窮舉小群組內所有可能排列，不是抽樣模擬）算出每個人抽中第幾志願（或落空）的機率，姓名旁若有落空風險會加註 ⚠ 符號。

機率計算的邏輯抽成共用檔案 `public/lottery-prob.js`，`example.html` 與 `admin.html`（後台「可能落空」預估，見下方「學生管理」）共用同一套，避免邏輯重複維護。這套計算是**展示 / 預估用**，概念上與正式抽籤的 `computeAllocation`（`functions/_lib.js`）一致，但屬於獨立實作——不是正式抽籤依據，正式結果一律以後台抽籤紀錄與 `verify-lottery.html` 的驗證為準。
...
- 匯出 CSV（含備註欄位）
- 抽籤前（抽籤制、尚未抽籤時）顯示「可能落空」預估：統計卡顯示風險人數，學生姓名旁加註 ⚠ 徽章，滑鼠移上去看確切機率，方便主動提醒（僅供參考，非正式抽籤依據）
