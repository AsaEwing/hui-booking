...
- 抽籤前，`result.html` 顯示的是「登記狀況」（每個位置目前有誰登記，不代表最終結果）；抽籤完成後才顯示正式結果。
...
專案可設定 `active_until`，超過此時間後 `result.html` 會自動視為關閉（不需要額外排程機制，讀取當下即時比對時間點判斷）。設定值必須晚於截止時間（`close_at`）至少 3 天。
